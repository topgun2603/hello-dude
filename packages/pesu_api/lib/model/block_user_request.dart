...
class BlockUserRequest {
  ...
  static BlockUserRequest? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'userId'), 'Required key "BlockUserRequest[userId]" is missing from JSON.');
        assert(json[r'userId'] != null, 'Required key "BlockUserRequest[userId]" has a null value in JSON.');
        return true;
      }());
