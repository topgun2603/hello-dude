...
class TokenPair {
  ...
  @override
  String toString() => 'TokenPair[accessToken=$accessToken, refreshToken=$refreshToken, expiresInSeconds=$expiresInSeconds]';
  ...
  static TokenPair? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        ...
        assert(json.containsKey(r'expiresInSeconds'), 'Required key "TokenPair[expiresInSeconds]" is missing from JSON.');
        assert(json[r'expiresInSeconds'] != null, 'Required key "TokenPair[expiresInSeconds]" has a null value in JSON.');
        return true;
      }());
