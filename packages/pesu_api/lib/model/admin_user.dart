...
class AdminUser {
  ...
  @override
  String toString() => 'AdminUser[id=$id, displayName=$displayName, phone=$phone, role=$role, status=$status, primaryLanguage=$primaryLanguage, createdAt=$createdAt, coins=$coins, earningsPaise=$earningsPaise, calls=$calls, reportsAgainst=$reportsAgainst, kycStatus=$kycStatus, online=$online, takingCalls=$takingCalls, lastSeenAt=$lastSeenAt, avatarId=$avatarId]';
  ...
  static AdminUser? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        ...
        assert(json.containsKey(r'takingCalls'), 'Required key "AdminUser[takingCalls]" is missing from JSON.');
        assert(json[r'takingCalls'] != null, 'Required key "AdminUser[takingCalls]" has a null value in JSON.');
        assert(json.containsKey(r'lastSeenAt'), 'Required key "AdminUser[lastSeenAt]" is missing from JSON.');
        assert(json.containsKey(r'avatarId'), 'Required key "AdminUser[avatarId]" is missing from JSON.');
        assert(json[r'avatarId'] != null, 'Required key "AdminUser[avatarId]" has a null value in JSON.');
        return true;
      }());
