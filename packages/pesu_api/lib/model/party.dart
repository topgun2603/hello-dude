...
class Party {
  ...
  @override
  String toString() => 'Party[id=$id, displayName=$displayName, avatarId=$avatarId]';
  ...
  static Party? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'id'), 'Required key "Party[id]" is missing from JSON.');
        assert(json[r'id'] != null, 'Required key "Party[id]" has a null value in JSON.');
        assert(json.containsKey(r'displayName'), 'Required key "Party[displayName]" is missing from JSON.');
        assert(json[r'displayName'] != null, 'Required key "Party[displayName]" has a null value in JSON.');
        assert(json.containsKey(r'avatarId'), 'Required key "Party[avatarId]" is missing from JSON.');
        assert(json[r'avatarId'] != null, 'Required key "Party[avatarId]" has a null value in JSON.');
        return true;
      }());
