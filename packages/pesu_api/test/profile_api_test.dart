//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//
// @dart=2.18

// ignore_for_file: unused_element, unused_import
// ignore_for_file: always_put_required_named_parameters_first
// ignore_for_file: constant_identifier_names
// ignore_for_file: lines_longer_than_80_chars

import 'package:pesu_api/api.dart';
import 'package:test/test.dart';


/// tests for ProfileApi
void main() {
  // final instance = ProfileApi();

  group('tests for ProfileApi', () {
    //Future<Profile> getMe() async
    test('test getMe', () async {
      // TODO
    });

    // Languages users can pick
    //
    //Future<List<ListLanguages200ResponseInner>> listLanguages() async
    test('test listLanguages', () async {
      // TODO
    });

    // Register this phone's FCM token for call and message pushes
    //
    //Future<String> registerDevice(RegisterDeviceRequest registerDeviceRequest) async
    test('test registerDevice', () async {
      // TODO
    });

    // Update name, avatar or languages (primary language must be in `languages`)
    //
    //Future<Profile> updateMe(UpdateMeRequest updateMeRequest) async
    test('test updateMe', () async {
      // TODO
    });

  });
}
