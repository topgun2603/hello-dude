//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//
// @dart=2.18

// ignore_for_file: unused_element, unused_import
// ignore_for_file: always_put_required_named_parameters_first
// ignore_for_file: constant_identifier_names
// ignore_for_file: lines_longer_than_80_chars

import 'package:pesu_api/api.dart';
import 'package:test/test.dart';

// tests for AdminPromotionInput
void main() {
  // final instance = AdminPromotionInput();

  group('test AdminPromotionInput', () {
    // int id
    test('to test the property `id`', () async {
      // TODO
    });

    // String title
    test('to test the property `title`', () async {
      // TODO
    });

    // String body
    test('to test the property `body`', () async {
      // TODO
    });

    // String highlight
    test('to test the property `highlight`', () async {
      // TODO
    });

    // String badge
    test('to test the property `badge`', () async {
      // TODO
    });

    // String emoji
    test('to test the property `emoji`', () async {
      // TODO
    });

    // String ctaLabel
    test('to test the property `ctaLabel`', () async {
      // TODO
    });

    // Where the button goes inside the app
    // String ctaAction
    test('to test the property `ctaAction`', () async {
      // TODO
    });

    // String theme
    test('to test the property `theme`', () async {
      // TODO
    });

    // bool confetti
    test('to test the property `confetti`', () async {
      // TODO
    });

    // Show a countdown when set
    // Object endsAt
    test('to test the property `endsAt`', () async {
      // TODO
    });

    // never_paid / paid = callers with no / at least one credited coin purchase
    // String audience
    test('to test the property `audience`', () async {
      // TODO
    });

    // every_open = each time the app opens; daily = once per IST day; once = only ever once
    // String frequency
    test('to test the property `frequency`', () async {
      // TODO
    });

    // int priority
    test('to test the property `priority`', () async {
      // TODO
    });

    // bool isActive
    test('to test the property `isActive`', () async {
      // TODO
    });

    // Object startsAt
    test('to test the property `startsAt`', () async {
      // TODO
    });

    // String status
    test('to test the property `status`', () async {
      // TODO
    });

    // int shown
    test('to test the property `shown`', () async {
      // TODO
    });

    // int clicked
    test('to test the property `clicked`', () async {
      // TODO
    });

    // int shownToday
    test('to test the property `shownToday`', () async {
      // TODO
    });

    // int clickedToday
    test('to test the property `clickedToday`', () async {
      // TODO
    });

    // Different people who saw it
    // int reach
    test('to test the property `reach`', () async {
      // TODO
    });

    // Object createdAt
    test('to test the property `createdAt`', () async {
      // TODO
    });


  });

}
