...
class UnreadCount extends Notifier<int> {
  ...
  int build() {
    ...
    _sub = ref.read(realtimeProvider).events.where((e) => e['t'] == 'notification').listen((_) => state = state + 1);
  ...
  Future<void> refresh() async {
    ...
    try {
      state = (await api.call(() => api.notifications.unreadNotificationCount())).unread;
    } catch (_) {
...
final notificationsProvider = FutureProvider.autoDispose<ListNotifications200Response>((ref) {
  final api = ref.read(apiProvider);
...
class NotificationsScreen extends ConsumerWidget {
  ...
  Future<void> _markRead(WidgetRef ref, List<int>? ids) async {
    final api = ref.read(apiProvider);
    try {
      final r = await api.call(() => api.notifications.markNotificationsRead(MarkNotificationsReadRequest(ids: ids ?? [], all: ids == null)));
              ...
              Padding(
                ...
                child: Row(
                  children: [
                    ...
                    Expanded(child: Text('Notifications', style: AppText.heading(22))),
                    if (hasUnread)
                      TextButton(
                        onPressed: () => _markRead(ref, null),
                        child: Text('Read all', style: AppText.body(14, color: AppColors.pinkSoft, weight: FontWeight.w700)),
                      ),
                  ],
                ),
              ),
                  ...
                  child: list.when(
                    ...
                    data: (data) => data.items.isEmpty
                        ? ListView(children: [
                            ...
                            const Icon(Icons.notifications_none_rounded, size: 56, color: AppColors.hint),
                            const SizedBox(height: 12),
                            Text('No notifications yet', textAlign: TextAlign.center, style: AppText.heading(18)),
                        ...
                        : ListView(
                            ...
                            children: [
                              for (final (label, items) in groupByDay(data.items, DateTime.now())) ...[
                                Padding(
                                  padding: const EdgeInsets.only(top: 14, bottom: 6),
                                  child: Text(label.toUpperCase(),
                                      style: AppText.body(12, color: AppColors.hint, weight: FontWeight.w700).copyWith(letterSpacing: 1.2)),
                                ),
...
List<(String, List<NotificationItem>)> groupByDay(List<NotificationItem> items, DateTime now) {
  final today = DateTime(now.year, now.month, now.day);
  final yesterday = today.subtract(const Duration(days: 1));
  final groups = <String, List<NotificationItem>>{};
  for (final n in items) {
    final t = n.createdAt.toLocal();
    final label = !t.isBefore(today) ? 'Today' : !t.isBefore(yesterday) ? 'Yesterday' : 'Earlier';
...
(IconData, Color) _iconFor(NotificationItemTypeEnum type) => switch (type) {
  NotificationItemTypeEnum.favouriteOnline => (Icons.favorite_rounded, AppColors.pink),
  ...
  NotificationItemTypeEnum.bookingReminder => (Icons.event_available_rounded, AppColors.lilac),
  NotificationItemTypeEnum.refundDecided => (Icons.undo_rounded, AppColors.success),
  NotificationItemTypeEnum.dailyBonus => (Icons.local_fire_department_rounded, AppColors.warning),
  NotificationItemTypeEnum.rateCall => (Icons.star_rounded, Color(0xFFFACC15)),
  NotificationItemTypeEnum.referralJoined || NotificationItemTypeEnum.referralRewarded => (Icons.card_giftcard_rounded, Color(0xFF60A5FA)),
  NotificationItemTypeEnum.reportActioned => (Icons.shield_outlined, Color(0xFF7DD3FC)),
  NotificationItemTypeEnum.supportCoins || NotificationItemTypeEnum.bonusEarned => (Icons.toll_rounded, AppColors.warning),
  NotificationItemTypeEnum.payoutPaid => (Icons.account_balance_wallet_rounded, AppColors.success),
  NotificationItemTypeEnum.payoutFailed => (Icons.error_outline_rounded, AppColors.danger),
  NotificationItemTypeEnum.kycDecided => (Icons.verified_user_outlined, AppColors.success),
  NotificationItemTypeEnum.vip => (Icons.workspace_premium_rounded, Color(0xFFFDE68A)),
  NotificationItemTypeEnum.chatMessage => (Icons.chat_bubble_outline_rounded, AppColors.lilac),
  NotificationItemTypeEnum.roomLive => (Icons.graphic_eq_rounded, AppColors.pink),
              ...
              child: Column(
                ...
                children: [
                  Text(item.title,
                      ...
                      style: AppText.body(15, weight: item.read ? FontWeight.w500 : FontWeight.w700)),
                  const SizedBox(height: 2),
                  Text('${item.body} · ${timeLabel(item.createdAt, DateTime.now())}',
                      maxLines: 2,
                      overflow: TextOverflow.ellipsis,
            ...
            if (item.type == NotificationItemTypeEnum.favouriteOnline) ...[
              ...
              DecoratedBox(
                ...
                child: Padding(
                  ...
                  child: Text('Call', style: AppText.body(13, weight: FontWeight.w700)),
                ),
              ),
            ],
            if (!item.read) ...[
              const SizedBox(width: 10),
              Container(width: 9, height: 9, decoration: const BoxDecoration(color: AppColors.pink, shape: BoxShape.circle)),
            ],
          ],
        ),
      ),
