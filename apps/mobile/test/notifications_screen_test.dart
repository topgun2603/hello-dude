...
void main() {
  ...
  test(
    'groups into Today / Yesterday / Earlier and labels times like the design',
    () {
      final items = [
        item(
          3,
        ...
        item(
          2,
          NotificationItemTypeEnum.rateCall,
          'b',
      ...
      expect(groupByDay(items, now).map((g) => (g.$1, g.$2.length)).toList(), [
        ('Today', 1),
        ('Yesterday', 1),
        ('Earlier', 1),
      ]);
  ...
  for (final size in const [Size(1080, 2412), Size(720, 1280)]) {
    testWidgets('inbox renders rows, Call button and unread dots at $size', (
      tester,
    ) async {
      tester.view.physicalSize = size;
      tester.view.devicePixelRatio = size.width / 360;
      addTearDown(tester.view.reset);
      final data = ListNotifications200Response(
        unread: 1,
        items: [
          item(
            2,
      ...
      ProviderScope(
        overrides: [
          notificationsProvider.overrideWith(
            (ref) async => ListNotifications200Response(unread: 0, items: []),
          ),
        ],
        child: const MaterialApp(home: NotificationsScreen()),
      ),
