...
void main() {
  for (final size in const [Size(1080, 2412), Size(720, 1280)]) {
    testWidgets('messages list shows people, previews and unread counts at $size', (tester) async {
      tester.view.physicalSize = size;
      tester.view.devicePixelRatio = size.width / 360;
      addTearDown(tester.view.reset);
      final data = ListChats200Response(unread: 2, items: [
        Conversation(
          id: 'c1',
          other: ConversationOther(id: 'u1', displayName: 'Priya', avatarId: 1, role: ConversationOtherRoleEnum.companion, online: true),
  ...
  testWidgets('empty messages explains when chat opens up', (tester) async {
    await tester.pumpWidget(ProviderScope(
      overrides: [chatsProvider.overrideWith((ref) async => ListChats200Response(unread: 0, items: []))],
      child: const MaterialApp(home: ChatsScreen()),
    ));
    await tester.pumpAndSettle();
    expect(find.text('After a call, you can keep talking here.'), findsOneWidget);
  });
}
