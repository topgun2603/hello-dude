      ...
      for (var i = 1; i <= 7; i++)
        CheckInDaysInner(
          ...
          state: i < day
              ...
              ? (claimed
                    ? CheckInDaysInnerStateEnum.claimed
                    : CheckInDaysInnerStateEnum.today)
...
void main() {
  for (final size in const [Size(1080, 2412), Size(720, 1280)]) {
    testWidgets(
      'daily bonus: 7-day ladder, Day 4 streak, claim button at $size',
      (tester) async {
        await pumpAt(tester, size, const CheckInScreen(), [
          checkInProvider.overrideWith((ref) async => checkIn(day: 4)),
        ]);
    ...
    testWidgets('invite friends shows the code, reward and steps at $size', (
      tester,
    ) async {
      await pumpAt(tester, size, const ReferralScreen(), [
        referralProvider.overrideWith(
          (ref) async => GetReferral200Response(
            code: 'KARTHIK50',
    ...
    testWidgets(
      'share card never shows who, only minutes, language and code at $size',
      (tester) async {
        await pumpAt(tester, size, const ShareCardScreen(), [
          shareCardProvider.overrideWith(
            (ref) async => GetShareCard200Response(
              todayMinutes: 32,
              language: 'ta',
              code: 'KARTHIK50',
              link: 'https://hellodude.app/r/KARTHIK50',
        ...
        expect(
          find.text('of good conversation in Tamil today'),
          findsOneWidget,
        );
  ...
  testWidgets('already claimed today disables the button', (tester) async {
    await pumpAt(tester, const Size(1080, 2412), const CheckInScreen(), [
      checkInProvider.overrideWith(
        (ref) async => checkIn(day: 2, claimed: true),
      ),
    ]);
