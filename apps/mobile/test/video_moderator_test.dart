...
void main() {
  test('blurs on the first nudity frame, reports once per cooldown, clears after 3 clean frames', () async {
  ...
  test('a slow check is never overlapped, and failures never throw into the call', () async {
  ...
  test('model input is 224×224×3 floats in 0–1, and the nudity score is porn + hentai', () {
    final source = img.Image(width: 640, height: 480)..clear(img.ColorRgb8(255, 0, 128));
    ...
    expect(nudityScore([0.0, 0.0, 0.1, 0.0, 0.9]), 0); // swimwear alone is not nudity

    final small = shrinkForUpload(Uint8List.fromList(img.encodeJpg(img.Image(width: 1280, height: 720))));
