...
const SETTING_ICON: Record<string, { icon: React.ElementType; tint: string }> = {
  ...
  "payout.tds_no_pan_bps": { icon: IdCard, tint: "bg-amber-50 text-amber-600" },
  "referral.companion_bonus_paise": { icon: Gift, tint: "bg-emerald-50 text-emerald-600" },
...
const SETTING_VIEW: Record<string, { unit: string; toView: (v: number) => number; fromView: (v: number) => number; step: number; time?: boolean }> = {
  ...
  "payout.tds_no_pan_bps": { unit: "%", toView: (v) => v / 100, fromView: (v) => Math.round(v * 100), step: 0.1 },
  "referral.companion_bonus_paise": { unit: "₹", toView: (v) => v / 100, fromView: (v) => Math.round(v * 100), step: 1 },
